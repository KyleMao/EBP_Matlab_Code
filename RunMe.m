...
block_num = 0; % Initialize block to start from 0, generate new data set.
Runs = 3; % Number of blocks to generate.

% Select a learning algorithm.
prompt = 'Please Choose an algorithm:\n1. Binary-EBP \n2. Real-EBP\nChoosen algorithm: ';
result = input(prompt);
if result == 1
...
T_train = floor(size_data * (1-Test_ratio)); % Total number of samples in training set.
T_test = size_data - T_train; % Total number of samples in test set.
%T_test = floor(size_data * Test_ratio); % Version in original code.

epochs = 1; % Number of presentations of the training set in each run.
T = epochs * T_train; % Total number of patterns.
Rep_num = 1 / Test_ratio; % For K-fold cross valdiation, Rep_num == K.
...
% Network parameters.
R = [120];  % Number of hidden neurons.
    ...
    for ii = 1 : length(algorithm_array) % 'P','PP','BP','BPI','CP','SP','SP2'
        ...
        for jj = 1 : length(parameter_array)
            for rep = 1 : Rep_num
                ...
                if exist('b_cell_initial', 'var') % Initialize hidden biases.
                ...
                for pp = 1 : (P+1)
                    ...
                    switch algorithm
                        ...
                        case 'Real-EBP'
                            ...
                            [r, r_alt , h_cell, bias_cell] = ...
                                Real_EBP_minibatch(x, d, e, layers, h_cell, bias_cell, eta, w0_std, dropout, batch_size);
                        case'Binary-EBP'
                            eta = 1;
                            dropout = 0;
                            Task.algorithms.dropout = dropout;
                            ...
                            Task.algorithms.batch_size = batch_size;
                            [r, r_alt , h_cell,bias_cell] = ...
                                Binary_EBP_minibatch(x, d, e, layers, h_cell, bias_cell, eta, w0_std, dropout, batch_size);
                        otherwise
