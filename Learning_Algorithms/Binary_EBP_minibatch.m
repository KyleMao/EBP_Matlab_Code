function [r_out_cont, r_out, h_cell, bias_cell] ...
    = Binary_EBP_minibatch(x_in, d_in, enable, K, h0_cell, bias0_cell, eta, sigma_w, dropout, batch_size)
% BINARY_EBP_MINIBATCH EBP algorithm for multilayer neural network with binary weights with minibatch and dropout options
% x_in - TxM input, where T is time length (number of patterns to generate)
% d - TxN label
% enable -  Tx1 "enable training" flag 
% K - an L+1 long vector, with the l-th component being the width of the l-th layer, and L is the number of layers
...
for tt = 1 : batch_size_fixed : T
    batch_size = min(batch_size, T-tt); % Reduce batch size if not enough example left.
